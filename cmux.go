...
func (cmux *CMUX) Accept() (channel *CChannel, err error) {
	...
	//log.Printf(" len(cmux.newChannels) = %d", len(cmux.newChannels))
	if count == 0 {
		//log.Printf(">Accpet Block")
	...
	//log.Printf("Accept channel %v", channel.id)
...
func (cmux *CMUX) OpenStream() (channel *CChannel, err error) {
	...
	cmux.liveChannels.Store(channel.id, channel)
	//log.Printf("OpenStream %d", channel.id)
	return channel, nil
}
...
func (channel *CChannel) onRecv(p []byte) (n int, err error) {
	...
	if channel.writeTo != nil {
		//log.Printf("++Channel[%d] Write by push ", channel.id)
		channel.write2num += int64(len(p))
		n, err = channel.writeTo.Write(p)
	} else {
		cpyBuf := make([]byte, len(p))
		copy(cpyBuf, p)
		//log.Printf("++Channel[%d] Write To Buffer %d %d", channel.id, len(channel.buf), len(cpyBuf))
...
// func (channel *CChannel) ReadFrom(r io.Reader) (n int64, err error) {
...
func (channel *CChannel) Read(p []byte) (readed_n int, err error) {
	//log.Printf("channel[%d] direct read %d", channel.id, len(p))
	...
	if blen == readed_n {
		...
		//log.Printf("direct read-1 %d", readed_n)
	} else {
		//log.Printf("direct read-2 %d", readed_n)
		channel.buf[0] = channel.buf[0][readed_n:]
	}
	//log.Printf("direct %v", p[0:readed_n])
	return readed_n, nil
}
func (channel *CChannel) Write(wdata []byte) (writen_n int, err error) {
	//log.Printf("channel[%d] Write data %d  ", channel.id, len(wdata))
	var stackBuf [16]byte
	...
	for {
		...
		if size2write, err = channel.cmux.conn.Write(wdata[writen_n : writen_n+size2write]); err != nil {
			writen_n += size2write
			channel.cmux.onError()
			//log.Printf("Write error 2")
			return
		}
		//log.Printf("channel[%d] send data %d", channel.id, size2write, p[writen_n:writen_n+size2write])
...
func NewCChannel(cmux *CMUX, id uint32) (channel *CChannel, err error) {
	...
	//log.Printf(" channel %d,%d", totalChannel, totalChannelClosed)
	return channel, nil
}
